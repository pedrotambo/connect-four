...
def test_ai_doesnt_play_if_game_is_over(mocker):
...
def test_ai_selects_the_only_available_column_number(mocker):
    ...
    game_mock = mocker.MagicMock(available_column_numbers=[available_column_number], player_1_name="ai_name",
                                 current_player="ai_name", is_over=False)
    ai_player = ConnectFourRandomAIPlayer("ai_name", game_mock)
    ai_player.play()
